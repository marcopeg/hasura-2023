...
# Generate the specified number of demo events
fake_demo_events = [generate_fake_demo_event() for _ in range(LIMIT)]
...
payload = {
    "query": '''
    mutation insertFakeDemoEvents($objects: [fake_demo_events_insert_input!]!) {
      insert_fake_demo_events(objects: $objects) {
        affected_rows
        returning {
          created_at
          data
